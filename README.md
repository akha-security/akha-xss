<div align="center">
<img width="1407" height="397" alt="banner" src="<img width="1407" height="397" alt="banner" src="https://github.com/user-attachments/assets/262d9649-1368-4aa6-b51c-35953e406484" />
" />
...
- **Validation via Browser Engine:** Integrates with Playwright (Headless Chromium) to execute payloads and capture actual `alert` events, producing VERIFIED findings when execution is observed and LIKELY/POSSIBLE when confidence is lower.
...
| :--- | :--- | :--- |
| **80 - 100%** | **Confirmed** | Execution observed or strong DOM evidence with reproducible traces. |
...
Finding states map to confidence and evidence:
- **VERIFIED**: browser execution observed or high-confidence DOM evidence
- **LIKELY**: strong reflection with consistent re-verification
- **POSSIBLE**: partial signals or weak context confidence
- **NEEDS_MANUAL_REVIEW**: incomplete evidence or restricted replay
...
Installs AKHA with Playwright/Selenium support required for high-confidence execution verification.
```bash
...
### Windows Quick Setup
If you only want to use Windows with Python installed, run:
```powershell
.\install.ps1
...
The setup script creates `.venv`, installs all Python dependencies, installs AKHA in editable mode, and installs the Playwright Chromium runtime used for false-positive reduction.

### Mitmproxy Compatibility
If your Python environment already has `mitmproxy 12.2.x` and pip reports dependency conflicts, install the compatibility pins before installing AKHA:
```bash
pip install -r requirements-mitmproxy-compat.txt
pip install -e . -c requirements-mitmproxy-compat.txt
```

### Browser Runtime
Install the Chromium runtime used by Playwright verification:
```bash
python -m playwright install chromium
```

### Health Check
Use the built-in doctor command when installing on a new machine or when browser verification does not work:
```bash
akha-xss doctor
```

It checks Python version, required Python modules, Playwright Chromium launch capability, proxy environment hints, and output folder permissions.
...
Need a ready-to-copy command for a real target?
```bash
akha-xss presets
```

Profiles are real runtime controls:
- `quick`: smaller crawl, lower payload caps, lower concurrency, and heavy optional modules disabled for first-pass triage.
- `balanced`: default mode for normal bug bounty or internal testing.
- `deep`: larger crawl and deeper discovery for authorized lab or controlled scopes.

### WAF-Protected Targets
Large production targets behind Cloudflare, Akamai, Imperva, or similar WAF/anti-bot layers can run much slower than lab targets. This is expected: AKHA adapts around 403/429/503 responses, challenge pages, unstable reflections, retries, and backoff windows instead of blindly flooding the target.

For a first pass on WAF-heavy targets, start with a cautious profile:
```bash
akha-xss scan --url https://domain.com \
  --profile quick \
  --max-pages 100 \
  --risk-top-k 50 \
  --max-payloads-per-param 5 \
  --max-payloads-per-endpoint 30 \
  --threads 4 \
  --rate-limit 3 \
  --no-stored-xss \
  --no-mxss \
...
After the first report, widen the scan gradually by increasing `--risk-top-k`, enabling one heavy module at a time, or raising payload caps. This keeps traffic controlled and makes it easier to tell whether slowness comes from real attack surface, WAF throttling, or an overly broad scope.

By default AKHA can automatically switch into WAF cautious mode when it detects a high-confidence WAF/challenge layer. The report records whether this happened under **Scan Policy**, so users can understand why a protected target ran slower or with narrower limits. Disable this behavior only when you intentionally want full control:
```bash
akha-xss scan --url https://domain.com --no-auto-waf-cautious
```
...
```
Use the long `--oast` flag. `-oast` is parsed as `-o ast`, which means "write reports to the `ast` directory".
...
The HTML report is designed for practical remediation review, not just raw scanner output. It includes executive metrics, prioritized action items, CWE/OWASP mapping, CVSS-like scoring, description, impact, recommendation, reproduction steps, request/response evidence, highlighted payload hits, browser verification details, scan policy, and phase timing.

The JSON report carries the same enriched triage fields for CI/CD and vulnerability management ingestion.

See the sanitized [example report snapshot](docs/example-report.md) for the fields users can expect without exposing real target data.
...
- **Playwright browser verification** is available with configurable minimum score, per-page timeout, and isolated-context concurrency controls.
...
- Reports include a structured **replay artifact** with URL, method, parameter, payload, headers, and body context where available.
- Runtime sink tracing records dangerous JavaScript sink use during verification without treating sink traces alone as execution proof.
...
- Proxy health snapshots report success/failure counts, failure streaks, quarantine state, and latency hints in telemetry.
...
- Reports include **HTTP telemetry** (latency percentiles, status buckets, pool utilization, proxy pool health).
...
- Reports include **scan policy** and **phase timeline** so slow WAF-heavy scans are explainable.
...
# Verification
--browser-verify
--browser-verify-min-score 2
--browser-verify-timeout-ms 8000
--browser-verify-concurrency 3
