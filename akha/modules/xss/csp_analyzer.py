...
class CSPAnalyzer:
    ...
    def _domain_key(self, url: str) -> str:
        """Extract path-level cache key from URL"""
        try:
    ...
    def analyze(self, url: str, response=None) -> Dict:
        ...
        if response:
            ...
            if not csp_header and response.headers.get('Content-Security-Policy-Report-Only'):
            ...
            try:
                ...
                if not csp_header and resp.headers.get('Content-Security-Policy-Report-Only'):
        ...
        result = self._parse_csp(csp_header, is_report_only)
        self._cache[cache_key] = result  # Store under domain key
        return result
    
    def _parse_csp(self, csp_header: Optional[str], is_report_only: bool = False) -> Dict:
        ...
        if is_report_only:
            result['details'].append('CSP is Report-Only - it does not block execution')
            result['risk_level'] = 'report_only'
            # Fall through to parse what it WOULD block, but we leave xss_exploitable=True
    ...
    def is_xss_exploitable(self, csp_result: Dict, payload: str) -> bool:
        ...
        if csp_result.get('is_report_only'):
            return True
    ...
    def get_summary(self, csp_result: Dict) -> str:
        ...
        if level == 'report_only':
            return "CSP is Report-Only - XSS payloads will execute if reflected"
        elif level == 'weak':
