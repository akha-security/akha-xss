...
import uuid
import logging
from urllib.parse import urlparse, urljoin, parse_qs, urlencode, urlunparse, quote, quote_plus
...
from akha.modules.xss.blind_xss import BlindXSSInjector, normalize_collaborator_url
...
from akha.payloads.smart_engine import SmartPayloadEngine
from akha.payloads.validator import SmartValidator
try:
    from akha.utils.json_logger import bind_log_context
except Exception:
...
class XSSEngine:
    ...
    def __init__(self, http_client, payload_manager, learning_engine, config):
        ...
        configured_verify_timeout = int(getattr(config, 'browser_verify_timeout_ms', 0) or 0)
        self._execution_timeout_ms = configured_verify_timeout or getattr(config, 'timeout', 10) * 1000
        self._execution_max_concurrency = int(getattr(config, 'browser_verify_max_concurrency', 3) or 3)
        ...
        self._stored_payload_meta: Dict[str, Dict[str, Any]] = {}
    ...
    def _verification_stage_score(
        ...
        *,
        ...
        if reverify_ok:
            stage_score += 1
        if diff_result and (getattr(diff_result, 'has_suspicious', False) or getattr(diff_result, 'structure_changed', False)):
            stage_score += 1
        return stage_score

    def _validate_and_record_vuln(
        self,
        vuln: Optional[Dict],
        payload: str,
        probe_result: Dict,
        ...
        """Process verification result, update stats, record learning, and append to findings.
        Returns True if a valid vulnerability was added."""
        if vuln:
    ...
    def _browser_verification_skip_reason(
        self,
        *,
        location: str,
        stage_score: int,
        ...
        if not verification_url:
            return 'skipped_no_replay_url'
        if location in ('POST', 'json_body'):
            return 'skipped_post_body_no_headless_replay'
        if not bool(getattr(self.config, 'browser_execution_verify', False)):
            return 'skipped_browser_verify_disabled'
        min_stage_score = int(getattr(self.config, 'browser_verify_min_score', 2) or 2)
        if stage_score < min_stage_score:
            return 'skipped_stage_score_below_threshold'
        return None

    def _is_waf_block_response(self, response: Any, body: str = "") -> bool:
        """Best-effort challenge/block detector used to stop noisy payload loops."""
        ...
        if any(h in headers for h in (
            ...
            return status in (403, 406, 409, 418, 429, 503)

        if status not in (403, 406, 409, 418, 429, 503):
            return False
    ...
    def set_collaborator_url(self, collaborator_url: Optional[str]) -> None:
        """Ensure blind XSS injector is configured for the given collaborator URL."""
        if not collaborator_url:
            return
        normalized = normalize_collaborator_url(collaborator_url)
        self.config.collaborator_url = normalized
        if self.blind_injector and self.blind_injector.collaborator_url == normalized:
            return
        self.blind_injector = BlindXSSInjector(
            collaborator_url=normalized,
    ...
    def _run_blind_injections(self, url: str, param_name: str,
                              param_location: str,
                              param_context: Optional[Dict]) -> None:
        if not self.blind_injector:
            return
    ...
    def _post_follow_redirect(self, response, base_url: str, check_text: Optional[str] = None):
        """Delegate to injector's multi-hop redirect handling."""
        return self.injector._follow_redirects(response, base_url, check_text, max_hops=5)
    ...
    def _send_probe(self, url: str, param_name: str, location: str,
        ...
        try:
            if location == 'json_body':
                ...
                if not self._set_json_path(json_template, param_name, probe_string):
    ...
    def _build_storage_meta(self, url: str, param_name: str) -> Dict[str, Any]:
        prefix = str(getattr(self.config, 'stored_xss_marker_prefix', 'AKHA_STORED'))
        ttl = int(getattr(self.config, 'stored_xss_ttl_seconds', 604800) or 604800)
        trace_id = uuid.uuid4().hex[:12]
        expires_at = int(time.time()) + max(60, ttl)
        marker = f"{prefix}:{trace_id}:{expires_at}"
        cleanup_marker = f"{prefix}_CLEANUP:{trace_id}"
        attribution = f"akha:{param_name}"
        return {
            'trace_id': trace_id,
    ...
    def _decorate_payloads_for_storage(self, payloads: List[str], *, context: str,
                                       url: str, param_name: str) -> List[str]:
        """Attach expiration and cleanup markers to stored-XSS payloads."""
        decorated: List[str] = []
        for payload in payloads:
            if not payload:
                continue
            meta = self._build_storage_meta(url, param_name)
            marker_blob = f"{meta['marker']}|{meta['cleanup_marker']}|{meta['attribution']}"

            if context in ('html', 'comment'):
                tagged = f"{payload}<!--{marker_blob}-->"
            elif context == 'attribute':
                tagged = f"{payload} data-akha-trace=\"{marker_blob}\""
                ...
                tagged = f"{payload}{sep}akha_trace={marker_blob}"
            else:
                tagged = f"{payload}<!--{marker_blob}-->"

            self._stored_payload_meta[tagged] = meta
            decorated.append(tagged)
    ...
    def _get_targeted_payloads(
        ...
        if bool(getattr(self.config, 'stored_xss_enabled', True)):
            payloads = self._decorate_payloads_for_storage(
                payloads,
                context=context,
                url=url,
    ...
    def _verify_xss(self, url: str, param_name: str, payload: str,
                    ...
                    outcome: Optional[Dict[str, Any]] = None) -> Optional[Dict]:
        """
        ...
        try:
            if location == 'json_body':
                ...
                if param_context:
                    api_url = param_context.get('form_action', url)
                    json_template = copy.deepcopy(param_context.get('json_body', {}))
                if not self._set_json_path(json_template, param_name, payload):
            ...
            if self._is_waf_block_response(response, body):
                if outcome is not None:
                    outcome['status'] = 'waf_block'
                    outcome['status_code'] = getattr(response, 'status_code', None)
                return None
            ...
            if content_type and not any(ct in content_type for ct in ['text/', 'html', 'xml']):
                if not (allow_json and 'json' in content_type):
                    if outcome is not None:
                        outcome['status'] = 'unsupported_content_type'
                    return None
            ...
            payload_reflected_encoded = self._is_payload_reflected_encoded(body, payload)
            
            if not payload_reflected_raw and not has_marker and not payload_reflected_encoded:
                if outcome is not None:
                    outcome['status'] = 'not_reflected'
                return None

            if has_marker and not marker_is_new and not payload_reflected_raw:
                if outcome is not None:
                    outcome['status'] = 'stale_marker'
                return None
            ...
            if has_marker:
                ...
                if not marker_in_tag and not payload_reflected_raw and not payload_reflected_encoded:
                    if outcome is not None:
                        outcome['status'] = 'marker_not_in_tag'
                    return None
            
            if not has_marker and not payload_reflected_raw and not payload_reflected_encoded:
                if outcome is not None:
                    outcome['status'] = 'not_reflected'
                return None
            ...
            try:
                ...
                if has_marker and (self.verifier.verify_class in body2 or self.verifier.verify_id in body2):
                    ...
                    reverify_ok = True
                elif payload_reflected_encoded and self._is_payload_reflected_encoded(body2, payload):
                    reverify_ok = True
            except Exception as e:
                if getattr(self.config, 'verbose', False):
                    import traceback
            ...
            _consistency_samples = 3 if getattr(self.config, 'aggressive_mode', False) else 1
            ...
            stage_score = self._verification_stage_score(
                marker_in_tag=marker_in_tag,
                payload_reflected=bool(payload_reflected_raw or payload_reflected_encoded),
            ...
            # Browser navigation only reproduces GET-style reflection; POST/JSON bodies are not replayed.
            verification_url = test_url
            browser_skip_reason = self._browser_verification_skip_reason(
                location=location,
                stage_score=stage_score,
                verification_url=verification_url,
            )
            _browser_ok = browser_skip_reason is None

            if verification_url and _browser_ok:
                try:
                    if self.execution_verifier is None:
                        self.execution_verifier = ExecutionVerifier(
                            timeout_ms=self._execution_timeout_ms,
                            browser_engine='chromium',
                            max_concurrency=self._execution_max_concurrency,
                        )
                    exec_result = self.execution_verifier.verify(verification_url, payload)
                ...
                if bool(getattr(self.config, 'execution_verify_firefox', False)):
                    ...
                    try:
                        ...
                        ff_result = self.execution_verifier_firefox.verify(verification_url, payload)
                        ...
                        if self.config.verbose:
                            import traceback
                            traceback.print_exc()
            elif browser_skip_reason:
                browser_matrix['chromium']['error'] = browser_skip_reason
            ...
            result_dict = {
                ...
                'confidence_tier': scoring_result.severity.value,
                'verification_stage_score': stage_score,
                'bypass_technique': self._get_bypass_technique(payload),
                'proof': self._generate_proof(test_url, payload),
                'replay': self._generate_replay_artifact(
                    test_url,
                    payload,
                    location,
                    _post_data_for_report,
                ),
    ...
    def _consistency_check(
        ...
        for i in range(attempts):
            ...
            if i < attempts - 1:
                lo, hi = (0.05, 0.2) if attempts <= 2 else (0.25, 0.85)
    ...
    def _is_payload_reflected_encoded(self, html: str, payload: str) -> bool:
        """Detect encoded reflections that may still be exploitable in attributes/URLs."""
        encoded_html = html_lib.escape(payload)
        encoded_url = quote(payload)
        encoded_url_plus = quote_plus(payload)

        needles = [n for n in (encoded_html, encoded_url, encoded_url_plus) if n]
        if not needles:
            return False

        attr_re = re.compile(
            r"(?i)\b(href|src|action|srcdoc|data|xlink:href|style)\s*=\s*['\"][^'\"]*{needle}[^'\"]*['\"]"
        )

        for needle in needles:
            if needle in html:
                if attr_re.search(html.replace(needle, '{needle}')):
                    return True

        return False

    def _is_reflection_in_safe_context(self, html: str, payload: str) -> bool:
        """Delegate safe-context checks to Verifier to keep one source of truth."""
        try:
            return self.verifier._is_in_safe_context(html, payload)
        except Exception:
            logger.debug("Verifier safe-context delegation failed", exc_info=True)
            return False
    ...
    def scan(self, url: str, parameters: List[Dict], waf_name: Optional[str] = None,
        ...
        for param in parameters:
            ...
            self._run_blind_injections(url, param_name, param_location, param_context)
            ...
            if (probe_result.get('_no_reflection')
                    and not (self.config.aggressive_mode
                             or self.config.deep_scan
                             or getattr(self.config, 'api_mode', False))):
                if session:
                    session.mark_tested(url, param_name, param_location)
                continue
            ...
            found_vuln = False
            waf_cautious = bool(waf_name) and bool(getattr(self.config, 'waf_cautious_applied', False))
            waf_block_streak = 0
            endpoint_profile = self._classify_endpoint_profile(url, param_location)
            encoding_profile = self._classify_encoding_profile(probe_result)
            ...
            for payload in payloads:
                ...
                bind_log_context(payload_id=_inject_uid)
                ...
                with self._injected_lock:
                    meta = self._stored_payload_meta.get(payload) or self._stored_payload_meta.get(_tracked_payload)
                    self._injected_payloads.append({
                        ...
                        'uid': _inject_uid,
                        'storage_meta': meta or {},
                    })
                
                verify_outcome: Dict[str, Any] = {}
                vuln = self._verify_xss(url, param_name, payload, param_location, param_context,
                                        baseline_html=baseline_html, outcome=verify_outcome)
                if verify_outcome.get('status') == 'waf_block':
                    waf_block_streak += 1
                else:
                    waf_block_streak = 0
                
                is_added = self._validate_and_record_vuln(
                    vuln, payload, probe_result, waf_name, url,
                    endpoint_profile, encoding_profile, csp_result,
                    aggressive_threshold, base_threshold, vulnerabilities
                )
                ...
                if waf_cautious and waf_block_streak >= 3 and not self.config.aggressive_mode:
                    break
            
            if (not found_vuln and probe_result.get('reflected', False)
                    and not (waf_cautious and waf_block_streak >= 3)):
            ...
            # Blind XSS already injected before probe; do not repeat here.
        ...
        for url_data in crawled_urls:
            ...
            try:
                ...
                for inj in self._injected_payloads:
                    ...
                    payload = inj['payload']
                    meta = inj.get('storage_meta') or {}
                    marker_blob_found = False
                    if meta:
                        marker = meta.get('marker')
                        cleanup_marker = meta.get('cleanup_marker')
                        if marker and marker in body:
                            marker_blob_found = True
                        elif cleanup_marker and cleanup_marker in body:
                            marker_blob_found = True
                    ...
                    payload_reflected = self._is_payload_reflected_raw(body, payload)
                    payload_in_body = payload and payload in body
                    if payload_reflected and (uid_found or marker_found or payload_in_body or marker_blob_found):
                        cleanup_result = None
                        if bool(getattr(self.config, 'stored_xss_cleanup_enabled', True)) and meta:
                            if bool(getattr(self.config, 'stored_xss_cleanup_on_detect', True)):
                                cleanup_result = self._attempt_stored_cleanup(inj, meta)
                        stored_vulns.append({
                            'url': url,
                            'parameter': inj['param'],
                            'payload': payload,
    ...
    def _attempt_stored_cleanup(self, inj: Dict, meta: Dict) -> Dict[str, Any]:
        """Best-effort cleanup for stored XSS markers (GET-only to avoid side effects)."""
        cleanup_marker = meta.get('cleanup_marker')
        if not cleanup_marker:
            return {'attempted': False, 'reason': 'missing_cleanup_marker'}

        location = str(inj.get('location') or '').lower()
        if location not in ('get', 'query', 'url'):
            return {'attempted': False, 'reason': 'unsupported_location'}

        try:
            cleanup_url = Injector._build_query_url(inj['url'], inj['param'], cleanup_marker)
            resp = self.client.get(cleanup_url, timeout=min(self.config.timeout, 10))
            ok = resp is not None and getattr(resp, 'status_code', 0) < 400
            return {
                'attempted': True,
    ...
    def _generate_replay_artifact(
        ...
        """Generate a structured, reproducible replay artifact for reports."""
        method = 'GET'
        body = None
        headers = {
        ...
        return {
            ...
            'body': body,
            'notes': (
                'Replay only against systems you are authorized to test. '
                'Browser verification may be required for final confirmation.'
            ),
        }
