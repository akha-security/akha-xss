...
class Verifier:
    ...
    def verify_reflection(self, body: str, payload: str) -> VerificationResult:
        ...
        encoded_exploitable = quality in (
            ReflectionQuality.URL_ENCODED,
            ReflectionQuality.HTML_ENCODED,
            ReflectionQuality.DOUBLE_ENCODED,
        ) and self._is_encoded_reflection_exploitable(body, payload)

        if not has_marker and not payload_raw and not encoded_exploitable:
            return result
        ...
        if has_marker and not marker_in_tag and not payload_raw and not encoded_exploitable:
            return result

        result.marker_in_tag = marker_in_tag
        result.payload_reflected_raw = payload_raw
        result.verified = marker_in_tag or payload_raw or encoded_exploitable
    ...
    def _is_encoded_reflection_exploitable(self, body: str, payload: str) -> bool:
        """Detect potentially exploitable encoded reflections in attributes/URLs."""
        encoded_html = html.escape(payload)
        encoded_url = urllib.parse.quote(payload)
        encoded_url_plus = urllib.parse.quote_plus(payload)
        ...
        for needle in needles:
            if needle in body:
                if attr_re.search(body.replace(needle, '{needle}')):
                    return True
        return False
