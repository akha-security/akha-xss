...
class Crawler:
    ...
    TRACKING_PARAMS = frozenset({
        ...
        '_ga', '_gid', '_gac', '_gl',
        'mc_cid', 'mc_eid', 'ref', 'ref_src', 'spm', 'scid', 'igshid',
    })
    FACETED_NUMERIC_PARAMS = frozenset({
        'page', 'p', 'offset', 'start', 'from', 'cursor', 'next', 'prev',
        'limit', 'per_page', 'size', 'sort', 'order',
    })
    ...
    def __init__(self, http_client, config):
        ...
        self._crawler_aggressiveness = str(getattr(self.config, 'crawler_aggressiveness', 'balanced') or 'balanced').lower()
        self.visited = set()
    ...
    def crawl(self, start_url: str, progress=None, task=None) -> List[Dict]:
        ...
        try:
            ...
            waf_cautious = bool(getattr(self.config, 'waf_cautious_applied', False))
            ...
            try:
                ...
                if result:
                    ...
                    if (not waf_cautious) and (challenge_detected or getattr(self.config, 'dynamic_crawling', False)):
        ...
        while queue and len(self.results) < self.config.max_pages:
            ...
            if bool(getattr(self.config, 'waf_cautious_applied', False)):
                batch_limit = min(batch_limit, 12)
            ...
            max_workers = min(len(batch), self.config.threads)
            if bool(getattr(self.config, 'waf_cautious_applied', False)):
        ...
        if self.config.parse_js and self.js_files and not bool(getattr(self.config, 'waf_cautious_applied', False)):
    ...
    def _passive_discovery(self, start_url: str, base_domain: str) -> List[str]:
        ...
        if waf_cautious:
            urls.update(list(sitemap_urls)[:180])
        else:
        ...
        if waf_cautious:
            archive_urls = set(list(archive_urls)[:100])
    ...
    def _probe_common_paths(self, base_url: str, base_domain: str) -> Set[str]:
        ...
        if self._crawler_aggressiveness == 'low':
            path_limit = 20 if self.config.deep_scan else 12
        elif self._crawler_aggressiveness == 'high':
            path_limit = 80 if self.config.deep_scan else 60
        if bool(getattr(self.config, 'waf_cautious_applied', False)):
            path_limit = min(path_limit, 25)
        ...
        for path in self.SENSITIVE_PATHS:
            ...
            try:
                ...
                if resp.status_code == 200 and self.SENSITIVE_PATTERNS.search(resp.text):
                    finding = {'url': url, 'type': 'sensitive_file_exposed'}
    ...
    def _normalize_url(self, url: str) -> str:
        ...
        path = self._normalize_path_segments(parsed.path)
    ...
    def _normalize_path_segments(self, path: str) -> str:
        if not path or path == '/':
            return '/'
    ...
    def _normalize_param_value(self, value: str, key: str = "") -> str:
        if value is None:
            return ''
        value = str(value)
        if not value:
        ...
        if key_lower in self.FACETED_NUMERIC_PARAMS and re.match(r'^\d+$', value):
            return '{page}'
    ...
    def _normalize_query_params(self, query: str) -> str:
        ...
        for key in sorted(parsed.keys()):
            ...
            norm_values = []
            for v in values:
                nv = self._normalize_param_value(v, key)
                if nv not in norm_values:
                    norm_values.append(nv)
            if not norm_values:
                normalized_items.append(key)
            else:
                joined = ','.join(sorted(norm_values))
                normalized_items.append(f"{key}={joined}" if joined else key)
    ...
    def _submit_forms_for_discovery(self, page_url: str, html: str, base_domain: str) -> List[str]:
        ...
        if self._crawler_aggressiveness == 'low':
            form_limit = 4 if self.config.deep_scan else 3
        elif self._crawler_aggressiveness == 'high':
            form_limit = 16 if self.config.deep_scan else 10
