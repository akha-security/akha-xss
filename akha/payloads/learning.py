...
class LearningEngine:
    ...
    def __init__(self, config: Any) -> None:
        ...
        self._lock_shards = {
            "global": [threading.Lock() for _ in range(16)],
            "contexts": [threading.Lock() for _ in range(16)],
            "wafs": [threading.Lock() for _ in range(8)],
            "domains": [threading.Lock() for _ in range(16)],
            "similarity": [threading.Lock() for _ in range(16)],
        }
        self._dirty = False
        self.stats: Dict = self._load_stats()

    def _lock_for(self, scope: str, key: str) -> threading.Lock:
        shards = self._lock_shards[scope]
        return shards[hash(key) % len(shards)]

    @contextlib.contextmanager
    def _lock_many(self, locks: List[threading.Lock]):
        uniq = sorted({id(l): l for l in locks}.values(), key=id)
        for lock in uniq:
            lock.acquire()
        try:
            yield
        finally:
            for lock in reversed(uniq):
                lock.release()
    ...
    def flush(self) -> None:
        ...
        locks = [self._lock_for("global", "_flush")]
        with self._lock_many(locks):
            self._dirty = True
            self._save_stats()

    def __del__(self) -> None:
        """Automatically flush stats on garbage collection."""
        try:
            self.flush()
        except Exception:
            pass
    ...
    def record_success(self, payload: str, context: str,
        ...
        """
        locks = [self._lock_for("global", payload)]
        if context:
            locks.append(self._lock_for("contexts", context))
        if waf_name:
            locks.append(self._lock_for("wafs", waf_name))
        if domain:
            locks.append(self._lock_for("domains", _domain_key(domain)))
            if getattr(self.config, "payload_similarity_warm_start", True):
                for sim_key in _similarity_keys(domain):
                    locks.append(self._lock_for("similarity", sim_key))
    ...
    def get_ranked_payloads(self, domain: str,
        ...
        """
        locks = [self._lock_for("global", "_rank")]
        if context:
            locks.append(self._lock_for("contexts", context))
        if domain:
            locks.append(self._lock_for("domains", _domain_key(domain)))
    ...
    def get_best_payloads(self, context: Optional[str] = None,
        ...
        """
        locks = [self._lock_for("global", "_best")]
        if context:
            locks.append(self._lock_for("contexts", context))
        if waf_name:
            locks.append(self._lock_for("wafs", waf_name))
    ...
    def get_best_payloads_ucb(self, context: Optional[str] = None,
        ...
        locks = [self._lock_for("global", "_ucb")]
        if context:
            locks.append(self._lock_for("contexts", context))
        if waf_name:
            locks.append(self._lock_for("wafs", waf_name))
        ...
        with self._lock_many(locks):
            ...
            # Dynamic UCB exploration factor: normalize by sqrt(log(total_trials))
            # This reduces exploration as we gather more data
            exploration_norm = math.sqrt(math.log(max(total_trials, 2))) / math.sqrt(math.log(2))
            exploration = exploration / exploration_norm if exploration_norm > 0 else exploration
    ...
    def get_stats(self) -> Dict:
        ...
        locks = [self._lock_for("global", "_stats")]
        with self._lock_many(locks):
            g = self.stats["global"]
