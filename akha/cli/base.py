...
examples:
    ...
    subparsers.add_parser(
        "doctor",
        help="Check local installation health",
        description="Check Python, dependencies, Playwright browser runtime, proxy hints, and output permissions.",
        formatter_class=AkhaHelpFormatter,
    )

    subparsers.add_parser(
        "presets",
        help="Show recommended scan presets",
        description="Print ready-to-copy commands for common scan scenarios.",
        formatter_class=AkhaHelpFormatter,
    )
...
def _add_scan_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    detection.add_argument(
        ...
        default=50,
        metavar="N",
        help="Minimum confidence for DOM XSS findings (default: 50)",
    )
    ...
    detection.add_argument(
        "--crawler-aggressiveness",
        choices=["low", "balanced", "high"],
        default="balanced",
        help="Crawler aggressiveness preset (default: balanced)",
    )
    detection.add_argument(
    ...
    detection.add_argument(
        "--no-auto-waf-cautious",
        dest="auto_waf_cautious",
        action="store_false",
        help="Disable automatic cautious limits when a WAF is detected",
    )
    detection.add_argument(
        "--api-mode",
        action="store_true",
    ...
    detection.add_argument(
        "--no-stored-xss-cleanup",
        dest="stored_xss_cleanup_enabled",
        action="store_false",
        help="Disable stored XSS cleanup attempts",
    )
    detection.add_argument(
        "--no-stored-xss-cleanup-on-detect",
        dest="stored_xss_cleanup_on_detect",
        action="store_false",
        help="Disable cleanup attempts when stored XSS is detected",
    )
    detection.add_argument(
        "--browser-verify-min-score",
        type=int,
        default=2,
        metavar="N",
        help="Min verification stage score before Playwright (default: 2)",
    )
    detection.add_argument(
        "--browser-verify-timeout-ms",
        type=int,
        default=8000,
        metavar="MS",
        help="Per-page Playwright verification timeout in milliseconds (default: 8000)",
    )
    detection.add_argument(
        "--browser-verify-concurrency",
        type=int,
        default=3,
        metavar="N",
        help="Max parallel isolated browser verification contexts (default: 3)",
    )
    detection.add_argument(
        "--probe-sensitive",
        action="store_true",
    ...
    detection.add_argument(
        "--browser-verify",
        dest="browser_execution_verify",
        action="store_true",
        default=False,
        help="Enable Playwright execution verification per candidate (slow; install playwright + browsers)",
    )
    detection.add_argument(
        "--auto-install-playwright",
        action="store_true",
        default=False,
        help="Allow AKHA to install Playwright and Chromium automatically when browser support is missing",
    )
    detection.add_argument(
