...
from akha.utils.json_logger import setup_json_logging, bind_log_context
...
class ScanController:
    ...
    def attach_scanner(self, scanner):
    ...
    def _is_tty_available(self):
        """Check if TTY is available (not in WSL/Docker without TTY)"""
        try:
    ...
    def _keyboard_monitor(self):
        ...
        try:
            if sys.platform == 'win32':
                ...
                # Check if TTY is available before attempting to read from stdin
                if not self._is_tty_available():
                    # No TTY available (WSL/Docker without TTY) — fall back to signal handling only
                    while True:
                        with self._lock:
                            if self.stopped:
                                break
    ...
    def _skip_phase(self):
        """Skip the current phase and continue"""
        with self._lock:
            if self.stopped:
                return
            console.print("\n[bold yellow][!] Skip requested - moving to next phase...[/bold yellow]")
        if self._scanner:
            self._scanner._request_skip_phase()
...
class Scanner:
    ...
    def __init__(self, config: Config):
        ...
        if getattr(config, 'structured_logging', False):
            setup_json_logging(getattr(config, 'log_level', 'INFO'))
            scan_id = uuid.uuid4().hex
            target_id = (getattr(config, 'target_url', '') or '').strip()
            bind_log_context(scan_id=scan_id, target_id=target_id, worker_id=self._task_worker_id)
        ...
        self._skip_phase = threading.Event()
        ...
        self.controller.attach_scanner(self)

    def _request_skip_phase(self):
        self._skip_phase.set()
        try:
            self.crawler.stop()
        except Exception:
            logger.debug("Crawler stop for skip failed", exc_info=True)

    def _consume_skip_phase(self) -> bool:
        if self._skip_phase.is_set():
            self._skip_phase.clear()
            return True
        return False
    ...
    def _phase_timeline(self) -> List[Dict]:
        items = []
        for name, meta in self._module_metrics.items():
            items.append({
                'phase': name,
                'runs': int(meta.get('runs', 0) or 0),
                'duration_seconds': round(float(meta.get('duration_seconds', 0.0) or 0.0), 3),
                'errors': int(meta.get('errors', 0) or 0),
            })
        return items

    def _apply_waf_cautious_mode(self, waf_result: Dict):
        """Reduce scan cost after a strong WAF/challenge signal."""
        if not bool(getattr(self.config, 'auto_waf_cautious', True)):
            return
        if not waf_result or not waf_result.get('detected'):
            return
        confidence = int(waf_result.get('confidence') or waf_result.get('confidence_score') or 0)
        if confidence < 70:
            return
        if bool(getattr(self.config, 'waf_cautious_applied', False)):
            return

        self.config.waf_cautious_applied = True
        self.config.max_pages = min(int(getattr(self.config, 'max_pages', 1500) or 1500), 350)
        self.config.max_depth = min(int(getattr(self.config, 'max_depth', 3) or 3), 2)
        self.config.risk_priority_top_k = min(int(getattr(self.config, 'risk_priority_top_k', 300) or 300), 220)
        self.config.max_payloads_per_param = min(
            int(getattr(self.config, 'max_payloads_per_param', 0) or 5), 5
        )
        self.config.max_payloads_per_endpoint = min(
            int(getattr(self.config, 'max_payloads_per_endpoint', 0) or 30), 30
        )
        self.config.threads = min(int(getattr(self.config, 'threads', 10) or 10), 4)
        self.config.rate_limit = min(int(getattr(self.config, 'rate_limit', 10) or 10), 3)
        self.config.timeout = min(int(getattr(self.config, 'timeout', 10) or 10), 6)
        self.config.stored_xss_enabled = False
        self.config.test_mxss = False
        self.config.test_angular = False
        self.config.test_graphql = False
        self.config.dynamic_crawling = False
        self.config.stateful_spa_discovery = False
        self.config.parse_js = False
        self.config.crawler_aggressiveness = 'low'

        if not self.config.quiet:
            print_detail(
                "WAF cautious mode enabled: reduced crawl depth/pages, discovery probes, payload caps, threads, rate, and heavy modules."
            )
    ...
    def _build_reflected_task_queue(self, crawled_urls: List[Dict]) -> DistributedTaskQueue:
        ...
        def _task_key(url_data: Dict) -> str:
            url = url_data.get('url', '')
            method = (url_data.get('method') or 'GET').upper()
            params = url_data.get('parameters') or []
            parts = []
            for p in params:
                name = str(p.get('name', '') or '')
                loc = str(p.get('location', '') or '')
                form_action = str(p.get('form_action', '') or '')
                parts.append(f"{name}|{loc}|{form_action}")
            parts.sort()
            raw = f"{url}|{method}|{';'.join(parts)}"
            digest = hashlib.sha1(raw.encode('utf-8')).hexdigest()
            return f"reflected:{digest}"
    ...
    def _phase_waf_detection(self, target_url: str) -> Dict[str, Any]:
        ...
        if waf_result['detected']:
            ...
            self._apply_waf_cautious_mode(waf_result)
        else:
            if not self.config.quiet:
    ...
    def _phase_csp_analysis(self, target_url: str) -> Dict[str, Any]:
    ...
    def scan(self, target_url: str, scan_mode: str = "full") -> Dict:
        ...
        if self.interactsh:
            if self.interactsh.register():
                ...
                self.xss_engine.set_collaborator_url(self.config.collaborator_url)
            else:
                if not self.config.quiet:
        ...
        try:
            waf_result = self._phase_waf_detection(target_url)
            
            if self._interrupted.is_set():
                return self._handle_interruption(target_url, waf_result, [])
            
            csp_result = self._phase_csp_analysis(target_url)
            ...
            if scan_mode == "full":
                ...
                if getattr(self.crawler, 'sensitive_findings', None):
                    for finding in self.crawler.sensitive_findings:
                        url = finding.get('url', '')
                        vuln = {
                            'type': 'sensitive_file_exposed',
                            'url': url,
                            'parameter': 'file',
                            'status': 'Sensitive file exposure',
                            'severity_level': 'potential',
                            'confidence': 65,
                            'exploitability_score': 40,
                            'payload': '',
                            'proof': f"Sensitive file exposure detected at {url}",
                            'validated': True,
                            'context': {'Location': 'File'},
                        }
                        self.session.add_vulnerability(vuln)
                        if not self.config.quiet:
                            print_result(False, f"Sensitive file exposure: {url}")

                if self._consume_skip_phase():
                    if not self.config.quiet:
                        print_detail("Skipped crawling phase")
                ...
                skip_param_discovery = self._consume_skip_phase()
                if skip_param_discovery:
                    if not self.config.quiet:
                        print_detail("Skipped parameter discovery phase")
                else:
                    if not self.config.quiet:
                    ...
                    with Progress(
                        ...
                        with ThreadPoolExecutor(max_workers=param_workers) as executor:
                            ...
                            for future in as_completed(futures):
                                if self._interrupted.is_set() or self._skip_phase.is_set():
                                    break
                                try:
            ...
            skip_xss_phase = self._consume_skip_phase()
            if skip_xss_phase and not self.config.quiet:
                print_detail("Skipped XSS testing phase")
            
            vulns_found_count = len(self.session.vulnerabilities)
            if not skip_xss_phase:
                ...
                def _test_single_url(url_data):
                    ...
                    if self.config.include_patterns:
                        if not any(re.search(p, url_path, re.IGNORECASE) for p in self.config.include_patterns):
                            return []  # URL doesn't match any include pattern

                    if self.config.exclude_patterns:
                        if any(re.search(p, url_path, re.IGNORECASE) for p in self.config.exclude_patterns):
                ...
                dom_max = getattr(self.config, 'dom_max_urls', 50)
                dom_urls = crawled_urls[:dom_max]
                
                mxss_max = getattr(self.config, 'mxss_max_urls', 100)
                all_params_for_mxss = []
                if getattr(self.config, 'test_mxss', True) and scan_mode == "full":
                    for ud in crawled_urls[:mxss_max]:
                ...
                with Progress(
                    ...
                    if self.xss_engine.payloads_tested == 0 and not self._interrupted.is_set():
                        try:
                            fallback_param = [{
                    ...
                    if self.config.stored_xss_enabled and scan_mode == "full" and not self._interrupted.is_set():
                    ...
                    if getattr(self.config, 'test_mxss', True) and scan_mode == "full" and not self._interrupted.is_set():
                    ...
                    if getattr(self.config, 'test_angular', True) and scan_mode == "full" and not self._interrupted.is_set():
                    ...
                    if getattr(self.config, 'test_graphql', True) and scan_mode == "full" and not self._interrupted.is_set():
                        try:
                    ...
                    if getattr(self.config, 'test_websockets', False) and scan_mode == "full" and not self._interrupted.is_set():
            ...
            if not skip_xss_phase and self.xss_engine.payloads_tested == 0 and not self.config.quiet:
                print_result(
                    False,
                    "No payload requests were sent. Check target reachability, scope filters, and method flags (e.g. --test-post).",
                )
            elif not skip_xss_phase and vulns_found_count == 0 and not self.config.quiet:
    ...
    def _generate_reports(self, target_url: str, waf_result: Dict, 
        ...
        report_data = {
            ...
            'scan_policy': {
                ...
                'auto_waf_cautious': bool(getattr(self.config, 'auto_waf_cautious', True)),
                'waf_cautious_applied': bool(getattr(self.config, 'waf_cautious_applied', False)),
                'max_pages': int(getattr(self.config, 'max_pages', 0) or 0),
                'risk_top_k': int(getattr(self.config, 'risk_priority_top_k', 0) or 0),
                'rate_limit': int(getattr(self.config, 'rate_limit', 0) or 0),
                'threads': int(getattr(self.config, 'threads', 0) or 0),
            },
