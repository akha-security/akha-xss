...
class TestPayloadIntelligence(unittest.TestCase):
    ...
    def _engine(self):
        cfg = Config.default()
        filename = f"output/test_payload_intelligence_stats_{uuid.uuid4().hex}.json"
        self.test_files.append(filename)
        cfg.learning_data_file = filename
    ...
    def test_learning_respects_configurable_weights(self):
        cfg = Config.default()
        filename = f"output/test_payload_weight_stats_{uuid.uuid4().hex}.json"
        self.test_files.append(filename)
        cfg.learning_data_file = filename
