...
class TestPhase3HttpWafAdaptation(unittest.TestCase):
    ...
    def test_waf_detection_is_passive_first_when_yaml_signature_matches(self):
        client = _WafClientStub(_Resp(headers={"cf-ray": "abc", "server": "cloudflare"}))
        ...
        self.assertEqual(result["detection_mode"], "passive")
        self.assertFalse(result["active_probing"])
        self.assertEqual(client.urls, ["https://example.com"])
        self.assertFalse(any("<script" in url or "OR+1%3D1" in url for url in client.urls))

    def test_yaml_profiles_extend_waf_signatures(self):
        detector = WAFDetector(_WafClientStub(_Resp()))

        self.assertIn("cloudflare", detector.yaml_profiles)
        self.assertIn("cf-ray", detector.signatures["cloudflare"]["headers"])
        self.assertIn("payloads", detector.yaml_profiles["cloudflare"])
