...
class _WAFBlockResp(_EchoResp):
    def __init__(self):
        super().__init__("<html><title>Attention Required</title>Cloudflare challenge</html>")
        self.status_code = 403
        self.reason = "Forbidden"
        self.headers = {"Content-Type": "text/html", "cf-ray": "unit-test"}


class _WAFBlockAfterProbeClient:
    """Reflects harmless probes, then blocks payload attempts like a WAF challenge."""

    def get(self, url, **kwargs):
        parsed = urlparse(url)
        qs = parse_qs(parsed.query)
        reflected = " ".join(vals[0] for vals in qs.values() if vals)
        if "akhaPROBE" in reflected:
            return _EchoResp(f"<html><body>{reflected}</body></html>")
        return _WAFBlockResp()
...
class TestRegressions(unittest.TestCase):
    ...
    def test_waf_cautious_reflected_scan_stops_after_repeated_blocks(self):
        cfg = Config.default()
        cfg.verbose = False
        cfg.learning_enabled = False
        cfg.aggressive_mode = False
        cfg.waf_cautious_applied = True
        cfg.max_payloads_per_param = 10

        engine = XSSEngine(
            _WAFBlockAfterProbeClient(),
            _DummyPayloadManager(),
            _DummyLearningEngine(),
            cfg,
        )
        ...
        # One counted probe plus three blocked payload attempts; the consistency
        # re-probe is intentionally not part of the payload counter.
        self.assertLessEqual(engine.payloads_tested, 4)
