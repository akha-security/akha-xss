...
class TestGoldenTargetRegression(unittest.TestCase):
    def test_json_report_contains_evidence_chain_and_telemetry(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("triage_summary", vulns[0])
            self.assertIn("reproduction_steps", vulns[0])
            self.assertIn("exploit_path", vulns[0])
            self.assertIn("description", vulns[0])
            self.assertIn("impact", vulns[0])
            self.assertIn("primary_recommendation", vulns[0])
            self.assertIn("classification", vulns[0])
            self.assertIn("cvss_like_score", vulns[0])
            self.assertEqual(vulns[0]["classification"].get("cwe"), "CWE-79")
            self.assertGreaterEqual(len(vulns[0]["reproduction_steps"]), 1)
            ...
            self.assertIn("phase_timeline", out)
            self.assertIn("scan_policy", out)
