...
class TestPhase1VerificationEnhancements(unittest.TestCase):
    ...
    def test_verify_result_contains_browser_matrix_and_exploitability(self):
        ...
        self.assertEqual(result['replay']['method'], 'GET')
        self.assertEqual(result['replay']['payload'], payload)

    def test_replay_artifact_preserves_post_body(self):
        cfg = Config.default()
        engine = XSSEngine(_DummyHttp(""), _DummyPayloadManager(), _DummyLearning(), cfg)

        replay = engine._generate_replay_artifact(
        ...
        self.assertEqual(replay["method"], "POST")
        self.assertEqual(replay["body"]["comment"], "<svg onload=alert(1)>")
        self.assertEqual(replay["headers"]["Content-Type"], "application/x-www-form-urlencoded")
