...
def test_blank_query_parameters_are_preserved_for_url_mode():
    finder = ParamFinder(_Client("<html></html>", "text/html"), _cfg())

    params = finder.find_parameters("https://example.com/search?q=&empty")

    by_name = {p["name"]: p for p in params}
    assert "q" in by_name
    assert "empty" in by_name
    assert by_name["q"]["value"] == ""
    assert by_name["empty"]["value"] == ""


def test_waf_cautious_mode_skips_expensive_param_fuzzing():
    cfg = _cfg(scan_profile="balanced")
    cfg.waf_cautious_applied = True
    finder = ParamFinder(_Client("<html></html>", "text/html"), cfg)

    assert finder._should_run_fuzzing("https://example.com/search?q=1", []) is False
