...
class TestCliConfigMapping(unittest.TestCase):
    def _args(self, **overrides):
        base = {
            ...
            "verified_only": False,
            "browser_execution_verify": False,
            "auto_install_playwright": False,
            "execution_verify_firefox": False,
            "browser_verify_timeout_ms": 8000,
            "browser_verify_concurrency": 3,
    ...
    def test_dash_oast_typo_is_rejected_before_ast_output_dir(self):
        args = self._args(output="ast", oast_enabled=False)

        cfg = _build_config(args)

        self.assertIsNone(cfg)

    def test_explicit_relative_ast_output_dir_is_allowed(self):
        args = self._args(output="./ast", oast_enabled=False)

        cfg = _build_config(args)

        self.assertIsNotNone(cfg)
        self.assertEqual(cfg.output_dir, "./ast")

    def test_proxy_mode_keeps_tls_verify_without_implicit_fallback(self):
        args = self._args(proxy="http://127.0.0.1:8080", quiet=True)

        cfg = _build_config(args)
        self.assertIsNotNone(cfg)
        self.assertTrue(cfg.verify_ssl)
        self.assertFalse(cfg.allow_ssl_fallback)

    def test_proxy_tls_fallback_requires_explicit_flag(self):
        args = self._args(
    ...
    def test_build_config_maps_phase3_new_flags(self):
        args = self._args(
            ...
            browser_verify_timeout_ms=12000,
            browser_verify_concurrency=4,
        ...
        self.assertEqual(cfg.browser_verify_timeout_ms, 12000)
        self.assertEqual(cfg.browser_verify_max_concurrency, 4)
    ...
    def test_browser_verification_runtime_knobs_are_clamped(self):
        args = self._args(
            browser_verify_timeout_ms=1,
            browser_verify_concurrency=99,
        )

        cfg = _build_config(args)

        self.assertEqual(cfg.browser_verify_timeout_ms, 2000)
        self.assertEqual(cfg.browser_verify_max_concurrency, 8)
    ...
    def test_auto_waf_cautious_flag_is_mapped(self):
        args = self._args(auto_waf_cautious=False)
        cfg = _build_config(args)
        self.assertIsNotNone(cfg)
        self.assertFalse(cfg.auto_waf_cautious)

    def test_quick_profile_applies_real_cost_limits(self):
        args = self._args(profile="quick")
        cfg = _build_config(args)
        self.assertIsNotNone(cfg)
        self.assertEqual(cfg.max_pages, 200)
        self.assertEqual(cfg.risk_priority_top_k, 80)
        self.assertEqual(cfg.max_payloads_per_param, 8)
        self.assertEqual(cfg.max_payloads_per_endpoint, 50)
        self.assertLessEqual(cfg.threads, 6)
        self.assertLessEqual(cfg.rate_limit, 5)
        self.assertFalse(cfg.stored_xss_enabled)
        self.assertFalse(cfg.test_mxss)
        self.assertFalse(cfg.test_angular)
        self.assertFalse(cfg.test_graphql)

    def test_deep_profile_expands_real_cost_limits(self):
        args = self._args(profile="deep")
        cfg = _build_config(args)
        self.assertIsNotNone(cfg)
        self.assertGreaterEqual(cfg.max_pages, 3000)
        self.assertGreaterEqual(cfg.risk_priority_top_k, 800)
        self.assertGreaterEqual(cfg.threads, 12)
        self.assertTrue(cfg.deep_scan)
